...
- ไปที่ **Logs** ของ Worker ดูข้อความ error ก่อนที่ instance จะล้ม (เช่น `REDIS_URL is not set`, `Redis connection error`, `Unhandled Rejection`, หรือ Prisma/DB error)
- ตรวจว่า **Environment** มี `REDIS_URL` และ `DATABASE_URL` ครบและค่าถูกต้อง
