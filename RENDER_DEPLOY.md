...
- **สาเหตุ:** Frontend กับ API อยู่คนละโดเมน (เช่น unified-line-inbox vs unified-line-oa-inbox) แต่ session cookie ไม่ถูกส่งข้ามโดเมน
- **แก้:** ไปที่ **API** service → **Environment** → ตั้ง `FRONTEND_URL` = URL จริงของ Frontend (เช่น `https://unified-line-inbox.onrender.com`) แล้ว Save เพื่อให้ Render redeploy API — โค้ดจะตั้ง cookie แบบ `sameSite: none` ให้ session ทำงานข้ามโดเมนได้
